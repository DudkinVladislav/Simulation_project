...
#ifdef AREA_PARALLEL_EXPORTS
#define AREA_PARALLEL_API __declspec(dllexport)
#else
#define AREA_PARALLEL_API __declspec(dllimport)
#endif
...
class AREA_PARALLEL_API Area_parallel : public Model {


public:

    Area_parallel();
    explicit Area_parallel(const std::string& model_name, const std::string& model_class);
    ~Area_parallel() override;
