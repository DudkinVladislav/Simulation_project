...
//  онструкторы
Area::Area()
    : Model("Area_test", "Area") {
}

Area::Area(const std::string& model_name, const std::string& model_class)
...
int Area::partition(vector<Node>& arr, int low, int high) {
...
void Area::quickSort(vector<Node>& arr, int low, int high) {
    if (low < high) {
...
void Area::modeling_transfer(Transfer& transf, Node node1, Node node2, double simulation_time)
{
...
double Area::collecting_metrics() {
    return 0;
}

double Area::modeling_way() {
    random_device rd;
    mt19937 gen(rd());
    map <string, double> params = get_all_parametrs();
    ...
    for (double i = get_parametr("start_number"); i <= get_parametr("max_number"); i += get_parametr("step_number"))
    {
        ...
        for (double j = get_parametr("start_radius"); j <= get_parametr("max_radius"); j += get_parametr("step_radius"))
        {
            ...
            Transfer* trans = new Transfer("transfer_" + to_string(i) + "_" + to_string(j), "moving_model");
            Average_number_transfered_messages* metric1 = new Average_number_transfered_messages("average_number_transfered_messages_" + to_string(i) + "_" + to_string(j), "average_number_transfered_messages");
            Average_number_transfered_messages_for_one_time* metric2 = new Average_number_transfered_messages_for_one_time("average_number_transfered_messages_for_one_time_" + to_string(i) + "_" + to_string(j), "average_number_transfered_messages_for_one_time");
            (*metric1).add_main_parametr("количество узлов", i);
            (*metric1).add_main_parametr("радиус", j);
            (*metric1).add_parametr("number_nodes", i);
            ...
            (*metric2).add_main_parametr("количество узлов", i);
            (*metric2).add_main_parametr("радиус", j);
...
extern "C" {
    ...
    __declspec(dllexport) void* CreateModel(const char* model_name, const char* model_class) {
        try {
            if (model_name && strlen(model_name) > 0 && model_class && strlen(model_class) > 0) {
                return new Area(model_name, model_class);
            }
            else {
                return new Area();
            }
    ...
    __declspec(dllexport) void DestroyModel(void* model) {
        if (model) {
            delete static_cast<Area*>(model);
        }
    }

    __declspec(dllexport) double ModelingWay(void* model) {
        if (model) {
            return static_cast<Area*>(model)->modeling_way();
        }
        return 0.0;
    }
    ...
    __declspec(dllexport) double CollectingMetric(void* model) {
        if (model) {
            return static_cast<Area*>(model)->collecting_metrics();
        }
        return 0.0;
    }
    ...
    __declspec(dllexport) string GetNextModelName(void* model) {
        if (model) {
            return static_cast<Area*>(model)->get_next_model_name();
        }
        else
            return "";
    }
    ...
    __declspec(dllexport) void AddComponent(void* model, const char* model_name, Model* current_model) {
        if (model) {
            static_cast<Area*>(model)->add_component(model_name, current_model);
        }
    }

    __declspec(dllexport) Model* GetComponent(void* model, const char* model_name) {
        if (model) {
            return static_cast<Area*>(model)->get_component(model_name);
        }
        else
            return nullptr;
    }
    ...
    __declspec(dllexport) int ComponentsSize(void* model) {
        if (model) {
            return static_cast<Area*>(model)->components_size();
        }
        return 0;
    }
    ...
    __declspec(dllexport) void AddMetric(void* model, Metric* metric) {
        if (model) {
            static_cast<Area*>(model)->add_metric(metric);
        }
    }

    __declspec(dllexport) Metric* GetMetric(void* model, int metric_nomer) {
        if (model) {
            return static_cast<Area*>(model)->get_metric(metric_nomer);
        }
        return nullptr;
    }
    ...
    __declspec(dllexport) int MetricsSize(void* model) {
        if (model) {
            return static_cast<Area*>(model)->metrics_size();
        }
        return 0;
    }
    ...
    __declspec(dllexport) int AllComponentsMetricsSize(void* model) {
        if (model) {
            return static_cast<Area*>(model)->all_components_metrics_size();
        }
        return 0;
    }
    ...
    __declspec(dllexport) void SetParametr(void* model, const char* param_name, double param_value) {
        if (model && param_name && param_value) {
            static_cast<Area*>(model)->set_parametr(param_name, param_value);
        }
    }

    __declspec(dllexport) double GetParametr(void* model, const char* param_name) {
        if (model && param_name) {
            return static_cast<Area*>(model)->get_parametr(param_name);
        }
        return 0.0;
    }
    ...
    __declspec(dllexport) void SetParallelParametr(void* model, const char* param_name, vector<double> param_value) {
        if (model && param_name && param_value.size() == 3) {
            static_cast<Area*>(model)->set_parallel_parametr(param_name, param_value);
        }
    }

    __declspec(dllexport) vector<double>* GetParallelParametr(void* model, const char* param_name) {
        if (model && param_name) {
            return static_cast<Area*>(model)->get_parallel_parametr(param_name);
        }
        else {
    ...
    __declspec(dllexport) void PrintName(void* model) {
        if (model) {
            static_cast<Area*>(model)->print_name();
        }
    }

    __declspec(dllexport) void PrintClass(void* model) {
        if (model) {
            static_cast<Area*>(model)->print_class();
        }
    }
    ...
    __declspec(dllexport) const char* GetName(void* model) {
        if (model) {
            ...
            name = static_cast<Area*>(model)->get_name();
            return name.c_str();
        }
        return "";
    }
    ...
    __declspec(dllexport) void MetricsOutput(void* model) {
        if (model) {
            static_cast<Area*>(model)->metrics_output();
        }
    }

    __declspec(dllexport) void DemonstrateModel(void* model) {
        if (model) {
            static_cast<Area*>(model)->demonstrate_model();
        }
    }

    __declspec(dllexport) vector <Metric*> GetAllComponentsMetrics(void* model) {
        if (model) {
            return static_cast<Area*>(model)->get_all_components_metrics();
        }
        else {
            vector <Metric*> empty;
            empty.resize(0);
    ...
    __declspec(dllexport) map <string, double> GetAllParametrs(void* model) {
        if (model) {
            return static_cast<Area*>(model)->get_all_parametrs();
        }
        else {
    ...
    __declspec(dllexport) int ParallelParametrsSize(void* model) {
        if (model) {
            return  static_cast<Area*>(model)->parallel_parametrs_size();
        }
        return 0;
    }
    ...
    __declspec(dllexport) map <string, vector<double>> GetAllParallelParametrs(void* model) {
        if (model) {
            return static_cast<Area*>(model)->get_all_parallel_parametrs();
        }
        else {
